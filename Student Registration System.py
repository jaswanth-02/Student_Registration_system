...
def Save():
    ...
    D1 = gender  # Assign gender from the global variable
    D2 = DOB.get()
    D1_date = Date.get()
    Re1 = Religion.get()
    S1 = Skill.get()
    ...
    if N1=="" or C1=="Select Branch" or D2=="" or Re1=="" or S1=="" or fathername=="" or mothername=="" or F1=="" or M1=="":
        messagebox.showerror("Error","Few Data is missing!")
    else:
        file = openpyxl.load_workbook('Student_data.xlsx')
        sheet = file.active
        sheet.append([R1, N1, C1, D1, D2, D1_date, Re1, S1, fathername, mothername, F1, M1])
...
#    print(x12)
    ...
    img=(Image.open("Student Images/"+str(x1)+".jpg")) #we done this to take image name same as reg_no
    resized_image=img.resize((190,190))
    photo2 = ImageTk.PhotoImage(resized_image)
    lbl.config(image=photo2)
    lbl.image=photo2
...
def Update():
    ...
    for row in sheet.rows:
        if row[0].value == R1:
            ...
            print(reg_number)
    #sheet.cell(column=1,row=int(reg_number),value=R1) now no one can update reg_no.
    sheet.cell(column=2,row=int(reg_number),value=N1)
    sheet.cell(column=3,row=int(reg_number),value=C1)
    sheet.cell(column=4,row=int(reg_number),value=D1)
    sheet.cell(column=5,row=int(reg_number),value=D2)
    sheet.cell(column=6,row=int(reg_number),value=D1_date)
    ...
    Clear()  #after update it will clear
...
# Initialize gender variable globally
gender = None

# Top frames
Label(root, text="Email: jaswanth7656@gmail.com", width=20, height=3, bg="#f0687c", anchor="e").pack(side=TOP, fill=X)
Label(root, text="STUDENT REGISTRATION", width=20, height=2, bg="#c36464", fg="#fff", font="arial 20 bold").pack(side=TOP, fill=X)
...
Branch = Combobox(obj, values=['Aerospace Engineering', 'Agricultural Engineering', 'Artificial Intelligence and Data Science',
                              'Biomedical Engineering', 'Biotechnology Engineering', 'Chemical Engineering', 'Civil Engineering',
                              'Computer Science and Engineering', 'Electrical and Electronics Engineering', 'Electrical Engineering',
                              'Electronics and Communication Engineering', 'Information Technology', 'Mechanical Engineering',
                              'Robotics Engineering'], font="Roboto 10", width=17, state="r")
Branch.place(x=630, y=50)
Branch.set("Select Branch")
